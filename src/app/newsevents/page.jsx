...
import { IoCalendarOutline, IoArrowForward, IoTimeOutline, IoPersonOutline } from 'react-icons/io5';
...
export default function NewsEventsPage() {
  ...
  useEffect(() => {
    const fetchNewsEvents = async () => {
      try {
        const response = await fetch('/api/posts?category=News%20%26%20Events');
        const data = await response.json();

        if (response.ok) {
          setPosts(data.posts || []);
        } else {
          setError(data.error || 'Failed to fetch news and events.');
        }
      } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="border-b border-slate-200 pb-8 space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 text-xs font-mono font-medium">
            <IoCalendarOutline size={14} />
            <span>CORPORATE UPDATES & CONFERENCES</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">
            News & Industry Events
          </h1>
          <p className="text-sm text-slate-600 max-w-2xl">
            Keep up with Pharmavive’s latest facility milestones, global symposium presentations, regulatory achievements, and corporate announcements.
          </p>
        </div>
        ...
        {error && !loading && (
          <div className="p-8 rounded-2xl bg-red-50 border border-red-200 text-center max-w-lg mx-auto space-y-3">
            <p className="text-sm font-bold text-red-800">Unable to load news</p>
        ...
        {!loading && !error && posts.length === 0 && (
          <div className="py-20 text-center bg-white rounded-3xl border border-slate-200 max-w-md mx-auto p-8 space-y-3">
            ...
            <h3 className="text-base font-bold text-slate-800">No corporate news yet</h3>
            <p className="text-xs text-slate-500">
              Check back soon for upcoming exhibition schedules, CPHI attendances, and company announcements.
            </p>
          </div>
        )}
        ...
        {!loading && !error && posts.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.map((post) => (
              <article
                key={post._id}
                className="group rounded-3xl bg-white border border-slate-200 overflow-hidden shadow-xs hover:border-emerald-300 hover:shadow-xl transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  <div className="relative h-52 w-full bg-slate-100 overflow-hidden">
                    {post.image ? (
                      ...
                      <div className="h-full w-full flex items-center justify-center text-slate-400 bg-slate-900">
                        <IoCalendarOutline size={40} className="text-emerald-500" />
                      </div>
                    )}
                  ...
                  <div className="p-6 space-y-3">
                    ...
                    <h2 className="text-lg font-bold text-slate-900 group-hover:text-emerald-700 transition-colors line-clamp-2">
                ...
                <div className="p-6 pt-0 border-t border-slate-100 mt-2">
                  <Link
                    href={`/posts/${post._id}`}
                    className="inline-flex items-center gap-2 text-xs font-semibold text-emerald-700 group-hover:text-emerald-900 transition-colors"
                  >
